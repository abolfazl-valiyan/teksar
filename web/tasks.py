...
#
#         if file_obj.filename.lower().endswith(('.mp3', '.wav', '.aac', '.flac', '.ogg', '.m4a')):
#             srt_text, txt_text = transcribe_audio_from_bytes(file_bytes, file_obj.filename)
#         elif file_obj.filename.lower().endswith(('.mp4', '.avi', '.mkv', '.mov', '.wmv', '.flv')):
#             srt_text, txt_text = transcribe_video_from_bytes(file_bytes, file_obj.filename)
#         else:
#             raise RuntimeError("فرمت فایل پشتیبانی نمی‌شود")
#
#         if file_obj.is_subtitle:
#             pass
#
#         if file_obj.is_text:
#             file_obj.transcribed_text = txt_text
#
